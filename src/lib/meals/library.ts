...
type Loaded = { library: MealLibrary; fetchedAt: string; sheetTitle: string };
...
async function loadFromSheets(): Promise<Loaded> {
  ...
  if (library.issues.length > 0) {
    console.warn(`[meals] ${library.issues.length} row(s) skipped or flagged in "${title}":`, library.issues);
  }
  console.info(`[meals] loaded ${library.meals.length} meals from "${title}" (gid tab resolved by API)`);
...
const NOT_CONFIGURED = "Brak konfiguracji Google Sheets (zmienne GOOGLE_* w .env.local).";
...
export async function getMealLibrary(): Promise<MealLibrarySnapshot> {
  if (!hasGoogleEnv()) return fallback(NOT_CONFIGURED);
  try {
