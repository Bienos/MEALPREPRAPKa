...
import { alternativesFor, buildPrep, finishPrep, regenerateShoppingList, swapDish } from "@/lib/meals/prep";

export type AlternativeView = { mealKey: string; mealName: string; kcal: number; protein_g: number };

/** [ ZBUDUJ PREP ] */
export async function buildPrepAction(days: { date: string; day_type: DayType }[]): Promise<void> {
  await buildPrep(days);
  refresh();
}
